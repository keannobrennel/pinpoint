...
export function calculatePriorityScore(aiAssessment, reportCount, reportedAt) {
  const severityWeight = (aiAssessment.severityScore / 100) * 50;
...
export function getZoneCentroid(boundaries) {
  if (!boundaries || boundaries.length === 0) return null;
  const sum = boundaries.reduce(
    (acc, p) => ({ lat: acc.lat + p.lat, lng: acc.lng + p.lng }),
    { lat: 0, lng: 0 },
  );
  return { lat: sum.lat / boundaries.length, lng: sum.lng / boundaries.length };
}
